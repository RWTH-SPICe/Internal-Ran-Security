...
SETUP=${SETUP:-"ipsec"}
ITERATIONS=${ITERATIONS:-250} # 250

IPSEC_AUTH_METHOD=${IPSEC_AUTH_METHOD:-"pubkey"} # pubkey, psk
IPSEC_IKE_CIPHER_SUITE=${IPSEC_IKE_CIPHER_SUITE:-"aes128gcm16-prfsha256-ecp256"} #aes128gcm16-prfsha256-ecp256
IPSEC_ESP_CIPHER_SUITE=${IPSEC_ESP_CIPHER_SUITE:-"aes128-sha2_256"} # aes128-sha2_256, aes256-sha2_256, aes256gcm16
IPSEC_START_ACTION=${IPSEC_START_ACTION:-"start"} # start, trap

DTLS_CIPHER=${DTLS_CIPHER:-"ECDHE-RSA-AES128-GCM-SHA256"}
DTLS_VERSION=${DTLS_VERSION:-"TLS1.2"}
DTLS_TYPE=${DTLS_TYPE:-RSA}
...
mkdir -p results
pcap_file_base="results/${SETUP}_$(date +"%Y-%m-%d-T%H-%M-%S")"
...
sudo nsenter -t "$PID_CUCP" -n tshark -i any -f "src net 192.168.72.0/28 or src net 192.168.200.0/24" -w "$pcap_file_cucp" &
tshark_pid_cucp=$!
sleep 5
reset
...
then
    write_vars_to_file "${pcap_file_base}.txt" SETUP ITERATIONS DTLS_CIPHER DTLS_VERSION
elif [[ "${SETUP}" == "ipsec" ]];
then
